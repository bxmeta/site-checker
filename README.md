...
| `/my_sites` | Список ваших сайтов | Все |
| `/status_all` | Статус всех сайтов | Только админы |
| `/check_now` | Мгновенная проверка | Только админы |
| `/sites` | Управление сайтами (интерактивное меню) | Только админы |
| `/add_site` | Добавить новый сайт (пошаговый мастер) | Только админы |
...
1. Ввод уникального ID (латинские буквы, цифры, подчёркивание)
2. Ввод названия сайта
3. Ввод URL
4. Выбор уровня поддержки
5. Ввод ожидаемого HTTP кода
6. Ввод ключевых слов (через запятую или `-` для пропуска)
7. Ввод Telegram ID подписчиков (через запятую или `-` для пропуска)
...
Статус поддержки: premium
Код: 200
Время: 01.01.2025 12:05:00 (UTC+4)
```
